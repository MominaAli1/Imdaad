...
| Client state | Zustand stores: `useImdaadStore` (civilian) + `useInstitutionStore` (war room) |
...
└── src/
    ...
    │   ├── maps/
    │   │   └── ImdaadMap.tsx         # MapLibre wrapper (theme + interactive props)
    │   └── ui/
...
`ImdaadMap.tsx` is a thin MapLibre GL wrapper. Renders:
- Incident pin
...
- The package name in `package.json` is `vaasta` (typo or branding pivot). Folder is `guardian`. Product is **Imdaad**. All three refer to the same thing.
- Leaflet AND MapLibre are both in dependencies. **MapLibre is the live one** (`ImdaadMap.tsx`). Don't get distracted by Leaflet.
